...
{
    public class FunctionRekognition
    {
        /// <summary>
        ...
        public FunctionRekognition()
        {
            ...
            if(!string.IsNullOrWhiteSpace(environmentMinConfidence))
            {
                ...
                if(float.TryParse(environmentMinConfidence, out value))
                {
                    ...
                    LambdaLogger.Log($"Setting minimum confidence to {this.MinConfidence}");
                }
                else
                {
                    LambdaLogger.Log($"Failed to parse value {environmentMinConfidence} for minimum confidence. Reverting back to default of {this.MinConfidence}");
                }
            }
            else
            {
                LambdaLogger.Log($"Using default minimum confidence of {this.MinConfidence}");
            }
        }
        ...
        public FunctionRekognition(IAmazonS3 s3Client, IAmazonRekognition rekognitionClient, float minConfidence)
        {
        ...
        public async Task FunctionHandler(S3Event input, ILambdaContext context)
        {
            foreach(var record in input.Records)
            {
                ...
                {
                    LambdaLogger.Log($"Object {record.S3.Bucket.Name}:{record.S3.Object.Key} is not a supported image type");
                    continue;
                }

                LambdaLogger.Log($"Looking for labels in image {record.S3.Object.Key} located in Bucket {record.S3.Bucket.Name}.");
                ...
                foreach(var label in detectResponses.Labels)
                {
                    if(tags.Count < 10)
                    {
                        LambdaLogger.Log($"\tFound Label {label.Name} with confidence {label.Confidence}");
                        tags.Add(new Tag { Key = label.Name, Value = label.Confidence.ToString() });
                    }
                    else
                    {
                        LambdaLogger.Log($"\tSkipped label {label.Name} with confidence {label.Confidence} because the maximum number of tags has been reached");
                    }
                }
